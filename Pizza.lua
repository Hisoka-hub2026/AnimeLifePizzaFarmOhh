    ...
    for _, gui in ipairs(guis) do
        ...
        for _, obj in ipairs(gui:GetDescendants()) do
            if (obj:IsA("TextButton") or obj:IsA("ImageButton")) then
                ...
                if matchFound and obj.Visible and obj.AbsoluteSize.X > 0 and obj.AbsoluteSize.Y > 0 then
                    if firesignal then
    ...
    if driveZone and driveZone:IsA("BasePart") and driveZone.Position.Y > -50 then
        while not isJobFinishedByGame and _G.AnimeLifeSmartAutofarm do
            ...
            if zoneFound and zoneVector then
                ...
                if pizzaBase then physicsTeleport(pizzaBase.Position) end
            end
        ...
        clickUiButtonByKeywords({"claim", "bank"})
        task.wait(0.5)
        clickUiButtonByKeywords({"close", "exit"})
        task.wait(1.5)
        restoreEnergy()
    else
        restoreEnergy()
...
local function createGUI()
    ...
    ToggleBtn.MouseButton1Click:Connect(function()
        ...
        if _G.AnimeLifeSmartAutofarm then
            ...
            LogLabel.Text = "Status: Running"
            task.spawn(function()
                while _G.AnimeLifeSmartAutofarm do
                    local success, err = pcall(runFullComboLoop)
                    if not success then
